...
function Zone.new(container)
	...
	self.zoneShape = enum.ZoneShape.Auto -- Determines optimal spatial query method
...
function Zone.fromRegion(cframe, size, zoneShape)
	...
	if zoneShape then
		zone.zoneShape = zoneShape
	end
	zone:relocate()
	return zone
end

function Zone.fromBox(cframe, size)
	-- Creates an optimized box-shaped zone using GetPartBoundsInBox
	-- Note: This calls fromRegion which auto-relocates the zone
	local zone = Zone.fromRegion(cframe, size, enum.ZoneShape.Box)
	return zone
end

function Zone.fromSphere(position, radius)
	-- Creates an optimized spherical zone using GetPartBoundsInRadius
	-- Creates a physical ball part for the zone
	local part = Instance.new("Part")
	...
	zone.zoneShape = enum.ZoneShape.Sphere
	zone.sphereRadius = radius
	zone.spherePosition = position
	-- Note: Call zone:relocate() manually if you want to move it to a WorldModel
	return zone
end
...
function Zone:_calculateRegion(tableOfParts, dontRound)
	...
	-- Convert bounds to CFrame + Size (modern approach instead of deprecated Region3)
	local regionSize = boundMax - boundMin
	local regionCFrame = CFrame.new((boundMin + boundMax) / 2)
	return regionCFrame, regionSize, boundMin, boundMax
end
...
function Zone:_update()
	...
	local regionCFrame, regionSize, boundMin, boundMax = self:_calculateRegion(zoneParts)
	local exactRegionCFrame, exactRegionSize, _, _ = self:_calculateRegion(zoneParts, true)
	self.regionCFrame = regionCFrame
	self.regionSize = regionSize
	self.exactRegionCFrame = exactRegionCFrame
	self.exactRegionSize = exactRegionSize
	self.boundMin = boundMin
	self.boundMax = boundMax
	self.volume = regionSize.X * regionSize.Y * regionSize.Z
...
function Zone:getParts()
	...
	local partsInRegion =
		self.worldModel:GetPartBoundsInBox(self.regionCFrame, self.regionSize, self.overlapParams.zonePartsIgnorelist)
...
function Zone:getRandomPoint()
	...
	local size = self.exactRegionSize
...
function Zone:setZoneShape(enumIdOrName)
	-- Allows changing the spatial query method used for this zone
	-- Options: "Auto", "Box", "Sphere"
	local enumId = tonumber(enumIdOrName)
	if not enumId then
		enumId = enum.ZoneShape[enumIdOrName]
		if not enumId then
			error(("'%s' is an invalid enumName!"):format(enumIdOrName))
		end
	else
		local enumName = enum.ZoneShape.getName(enumId)
		if not enumName then
			error(("%s is an invalid enumId!"):format(enumId))
		end
	end
	self.zoneShape = enumId
end
